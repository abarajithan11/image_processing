...
{ fprintf(stderr,"<<Cifi>>\n");
  ...
  if (!otimiza1) {
    ...
    for (int l=0; l<a.nl(); l++) {
      if (l%10==0 || l==a.nl()-1) fprintf(stderr,".");
...
{ VETOR<double> Y(X.n());
  ...
  coef=-infinito; ang=0;
  for (int i=0; i<X.n(); i++) {
    double coef2=ssimgry(Y,y2,mY,X,x2,mX,ssalpha,ssbeta,ssgama); 
...
{ fprintf(stderr,"<<Rafi>>\n");
  ...
  if (!otimiza2) {
    #pragma omp parallel for
    for (int l=0; l<ga.nl(); l++) {
      if (l%10==0 || l==ga.nl()-1) fprintf(stderr,".");
      for (int c=0; c<ga.nc(); c++) {
...
{ fprintf(stderr,"<<Tefi>>\n");
  ...
  if (!otimiza3) {
    ...
    for (int ax=ga.minx(); ax<=ga.maxx(); ax++) {
      if (modulo(ax,10)==0 || ax==ga.maxx()) fprintf(stderr,".");
...
{ using namespace CirateG;
  if (argc!=2 && argc!=5) {
    printf("CirateGS: Piramidal ciratefi for grayscale images v1.06\n");
    printf("  Written for OpenMP (parallel processing).\n");
